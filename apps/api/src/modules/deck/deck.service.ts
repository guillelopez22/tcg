...
import { eq, and, gt, ilike, inArray, sql, desc } from 'drizzle-orm';
...
export type MetaBreakdownEntry = {
  legendId: string;
  legendName: string;
  legendCleanName: string;
...
export class DeckService {
  ...
  async list(userId: string, input: DeckListInput): Promise<PaginatedResult<DeckWithCover>> {
    ...
    const flatRows = await this.db
      .select({
        ...
        domain: decks.domain,
        tier: decks.tier,
        tournament: decks.tournament,
        region: decks.region,
        placement: decks.placement,
        status: decks.status,
  ...
  async metaBreakdown(): Promise<MetaBreakdownEntry[]> {
    // Single query: join public decks -> deck_cards (legend zone) -> cards
    // Group by legend card, count decks, pick most common tier via mode aggregation
    const rows = await this.db
      .select({
        legendId: cards.id,
        legendName: cards.name,
        legendCleanName: cards.cleanName,
        legendImage: cards.imageSmall,
        deckCount: sql<number>`count(distinct ${decks.id})::int`,
        // Most common tier: aggregate to array, pick first non-null element after sorting by frequency
        // We use mode() ordered set aggregate for this
        tier: sql<string | null>`mode() within group (order by ${decks.tier})`,
      })
      .from(deckCards)
      .innerJoin(decks, and(
        eq(deckCards.deckId, decks.id),
        eq(decks.isPublic, true),
      ))
      .innerJoin(cards, and(
        eq(deckCards.cardId, cards.id),
        eq(cards.cardType, 'Legend'),
      ))
      .where(
        sql`${deckCards.zone} IN ('legend', 'champion')`,
      )
      .groupBy(cards.id, cards.name, cards.cleanName, cards.imageSmall)
      .orderBy(desc(sql`count(distinct ${decks.id})`));

    const totalDecks = rows.reduce((sum, r) => sum + r.deckCount, 0);

    return rows.map((r) => ({
      legendId: r.legendId,
      legendName: r.legendName,
      legendCleanName: r.legendCleanName,
      legendImage: r.legendImage,
      deckCount: r.deckCount,
      tier: r.tier ?? null,
      metaShare: totalDecks > 0 ? Math.round((r.deckCount / totalDecks) * 1000) / 10 : 0,
    }));
  }
